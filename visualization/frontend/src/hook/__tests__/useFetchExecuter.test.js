...
describe('useFetchExecutor hook', () => {
  ...
  it('should return error in case of failure', async () => {
    api = jest.fn().mockRejectedValue(new Error('error'));
    const { result } = renderHook(() => useFetchExecutor());

    await act(async () => {
      await expect(result.current.executeFetch(api)).rejects.toThrow(new Error('error'));
    });
  });
